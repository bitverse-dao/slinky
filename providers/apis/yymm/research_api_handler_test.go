...
func TestNewResearchAPIHandler(t *testing.T) {
	t.Run("fail if the name is incorrect", func(t *testing.T) {
		_, err := yymm.NewResearchAPIHandler(zap.NewNop(), config.APIConfig{
			Name: "incorrect",
		})
		require.Error(t, err)
	})
	...
	t.Run("test failure of api-config validation", func(t *testing.T) {
		cfg := yymm.DefaultResearchAPIConfig
		...
		_, err := yymm.NewResearchAPIHandler(zap.NewNop(), cfg)
		require.Error(t, err)
	})
	...
	t.Run("test success", func(t *testing.T) {
		_, err := yymm.NewResearchAPIHandler(zap.NewNop(), yymm.DefaultResearchAPIConfig)
		require.NoError(t, err)
	})
}

// TestCreateURL tests that:
//   - If no chain in the given chains are yymm - fail
//   - If one chain in the given chains is yymm - return the first endpoint configured
func TestCreateURLResearchHandler(t *testing.T) {
	ah, err := yymm.NewResearchAPIHandler(
		zap.NewNop(),
		yymm.DefaultResearchAPIConfig,
	)
	require.NoError(t, err)

	t.Run("non-yymm chains", func(t *testing.T) {
		chains := []types.Chain{
			{
				ChainID: "osmosis",
			},
	...
	t.Run("multiple chains w/ a yymm chain", func(t *testing.T) {
		chains := []types.Chain{
			{
				ChainID: "osmosis",
			},
			{
				ChainID: yymm.ChainID,
			},
		}

		url, err := ah.CreateURL(chains)
		require.NoError(t, err)
		require.Equal(t, yymm.DefaultResearchAPIConfig.Endpoints[1].URL, url)
	})
}
...
func TestParseResponseResearchAPI(t *testing.T) {
	...
	t.Run("fail if none of the chains given are yymm", func(t *testing.T) {
		chains := []types.Chain{
			{
				ChainID: "osmosis",
			},
	...
	t.Run("success", func(t *testing.T) {
		...
		researchJSON := yymmtypes.ResearchJSON{
			"1INCH": {
				ResearchJSONMarketParam: yymmtypes.ResearchJSONMarketParam{
					ID:                0,
					Pair:              "1INCH-USD",
					Exponent:          -10.0,
					MinPriceChangePpm: 4000,
					MinExchanges:      3,
					ExchangeConfigJSON: []yymmtypes.ExchangeMarketConfigJson{
						{
...
func TestParseResponseResearchCMCAPI(t *testing.T) {
	ah, err := yymm.NewResearchAPIHandler(
		zap.NewNop(),
		yymm.DefaultResearchCMCAPIConfig,
	)
	require.NoError(t, err)
