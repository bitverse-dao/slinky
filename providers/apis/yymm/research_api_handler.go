...
func NewResearchAPIHandler(
	...
	return &ResearchAPIHandler{
		...
		url:     api.Endpoints[1].URL, // We assume the first URL is the endpoint of the yymm mainnet
		onlyCMC: api.Name == ResearchCMCAPIHandlerName,
	}, nil
}

// ResearchAPIHandler is a subclass of the yymm_chain.ResearchAPIHandler. It interprets the yymm ResearchJSON
// as a market-map.
...
// CreateURL returns a static url (the url of the first configured endpoint). If the yymm chain is not
// configured in the request, an error is returned.
func (h *ResearchAPIHandler) CreateURL(chains []types.Chain) (string, error) {
	// expect at least one chain to be a yymm chain
	if _, ok := arrays.CheckEntryInArray(types.Chain{
		ChainID: ChainID,
	}, chains); !ok {
		return "", fmt.Errorf("yymm chain is not configured in request for the yymm research json")
	}

	return h.url, nil
}

// ParseResponse parses the response from the yymm ResearchJSON API into a MarketMap, and
// unmarshals the market-map in accordance with the underlying yymm ResearchAPIHandler.
func (h *ResearchAPIHandler) ParseResponse(
	...
	if !ok {
		h.logger.Error("yymm chain is not configured in request for the yymm research json")
	...
	// parse the response
	// unmarshal the response body into a yymm research json
	var research yymmtypes.ResearchJSON
	if err := json.NewDecoder(resp.Body).Decode(&research); err != nil {
		h.logger.Error("failed to parse yymm research json response", zap.Error(err))
		return types.NewMarketMapResponseWithErr(
			chains,
			providertypes.NewErrorWithCode(
				fmt.Errorf("failed to parse yymm research json response: %w", err),
				providertypes.ErrorFailedToDecode,
			),
		)
	}

	// convert the yymm research json into a QueryAllMarketsParamsResponse
	respMarketParams, err := h.researchJSONToQueryAllMarketsParamsResponse(research)
	if err != nil {
		h.logger.Error("failed to convert yymm research json into QueryAllMarketsParamsResponse", zap.Error(err))
		return types.NewMarketMapResponseWithErr(
			chains,
			providertypes.NewErrorWithCode(
				fmt.Errorf("failed to convert yymm research json into QueryAllMarketsParamsResponse: %w", err),
				providertypes.ErrorFailedToDecode,
			),
		)
	}
	...
	// resolve the response under the yymm chain
	resolved := make(types.ResolvedMarketMap)
	resolved[chain] = types.NewMarketMapResult(&marketMap, time.Now())

	h.logger.Debug("successfully resolved yymm research json into a market map", zap.Int("num_markets", len(marketMap.MarketMap.Markets)))
	return types.NewMarketMapResponse(resolved, nil)
}

// researchJSONToQueryAllMarketsParamsResponse converts a yymm research json into a
// QueryAllMarketsParamsResponse.
func (h *ResearchAPIHandler) researchJSONToQueryAllMarketsParamsResponse(research yymmtypes.ResearchJSON) (yymmtypes.QueryAllMarketParamsResponse, error) {
	// iterate over all entries in the research json + unmarshal it's market-params
	resp := yymmtypes.QueryAllMarketParamsResponse{}
	for _, market := range research {
		if h.onlyCMC && market.CMCID < 0 {
			continue
		}

		// convert the yymm research json market-param into a MarketParam struct
		marketParam, err := h.marketParamFromResearchJSONMarketParam(market)
		if err != nil {
			return yymmtypes.QueryAllMarketParamsResponse{}, err
		}
...
// marketParamFromResearchJSONMarketParam converts a yymm research json market-param
// into a MarketParam struct.
func (h *ResearchAPIHandler) marketParamFromResearchJSONMarketParam(marketParam yymmtypes.Params) (yymmtypes.MarketParam, error) {
	var exchangeConfigJSON yymmtypes.ExchangeConfigJson
	if !h.onlyCMC {
		exchangeConfigJSON = yymmtypes.ExchangeConfigJson{
			Exchanges: marketParam.ExchangeConfigJSON,
		}
	} else {
		exchange := yymmtypes.ExchangeMarketConfigJson{
			ExchangeName: coinmarketcap.Name,
			Ticker:       fmt.Sprintf("%d", marketParam.CMCID),
		}
		exchangeConfigJSON = yymmtypes.ExchangeConfigJson{
			Exchanges: []yymmtypes.ExchangeMarketConfigJson{exchange},
		}
	}
	// marshal to a json string
	exchangeConfigJSONBz, err := json.Marshal(exchangeConfigJSON)
	if err != nil {
		return yymmtypes.MarketParam{}, err
	}
