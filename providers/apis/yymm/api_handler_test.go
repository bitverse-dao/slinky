...
func TestCreateURL(t *testing.T) {
	handler, err := yymm.NewAPIHandler(zap.NewNop(), yymm.DefaultAPIConfig)
	...
	t.Run("single chain", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, fmt.Sprintf(yymm.Endpoint, yymm.DefaultAPIConfig.Endpoints[0].URL), url)
	})
}
...
func TestParseResponse(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			chains: chains[:1],
			resp: func() *http.Response {
				return testutils.CreateResponseFromJSON(yymmResponseInvalid)
			},
			expected: types.MarketMapResponse{
				UnResolved: types.UnResolvedMarketMap{
					chains[0]: providertypes.UnresolvedResult{
		...
		},
		{
			...
			chains: chains[:1],
			resp: func() *http.Response {
				return testutils.CreateResponseFromJSON(yymmResponseValid)
			},
			expected: types.MarketMapResponse{
