...
func TestDefaultSwitchOverProvider(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := yymm.NewDefaultSwitchOverMarketMapFetcher(tc.logger, tc.api, tc.rh, tc.metrics)
			if tc.err {
				require.Error(t, err)
				return
			}
...
func TestNewSwitchOverAPIHandler(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := yymm.NewSwitchOverFetcher(tc.logger, tc.pricesFetcher, tc.marketmapFetcher, metrics.NewNopAPIMetrics())
			if tc.err {
				require.Error(t, err)
				return
			}
...
func TestSwitchOverProvider_Fetch(t *testing.T) {
	...
	fetcher, err := yymm.NewSwitchOverFetcher(zap.NewNop(), pf, mmf, metrics)
	require.NoError(t, err)
	...
	cases := []struct {
	...
	}{
		{
			...
			pricesFetcher: func(pf *apihandlermocks.APIFetcher[mmclient.Chain, *mmtypes.MarketMapResponse]) {
				resp := mmclient.NewMarketMapResponse(
					mmclient.ResolvedMarketMap{
						yymm.YYMMChain: mmclient.NewMarketMapResult(
			...
			metrics: func(m *apimetricsmocks.APIMetrics) {
				m.On("AddProviderResponse", yymm.Name, strings.ToLower(yymm.YYMMChain.String()), providertypes.OK).Once()
			},
			resp: mmclient.NewMarketMapResponse(
				mmclient.ResolvedMarketMap{
					yymm.YYMMChain: mmclient.NewMarketMapResult(
		...
		},
		{
			...
			metrics: func(m *apimetricsmocks.APIMetrics) {
				m.On("AddProviderResponse", marketmap.Name, strings.ToLower(yymm.YYMMChain.String()), providertypes.OK).Once()
			},
			resp: mmclient.NewMarketMapResponse(
				mmclient.ResolvedMarketMap{
					yymm.YYMMChain: mmclient.NewMarketMapResult(
		...
		},
		{
			...
			marketmapFetcher: func(mmf *apihandlermocks.APIFetcher[mmclient.Chain, *mmtypes.MarketMapResponse]) {
				resp := mmclient.NewMarketMapResponse(
					...
					mmclient.UnResolvedMarketMap{
						yymm.YYMMChain: providertypes.UnresolvedResult{
			...
			metrics: func(m *apimetricsmocks.APIMetrics) {
				m.On("AddProviderResponse", marketmap.Name, strings.ToLower(yymm.YYMMChain.String()), providertypes.ErrorAPIGeneral).Once()
			},
			resp: mmclient.NewMarketMapResponse(
				make(mmclient.ResolvedMarketMap),
				mmclient.UnResolvedMarketMap{
