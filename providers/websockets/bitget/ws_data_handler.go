...
func (h *WebSocketHandler) HandleMessage(
	...
	if err := json.Unmarshal(message, &subscribeMessage); err == nil && subscribeMessage.Event == "subscribe" && subscribeMessage.Code == "" {
		responseMessage, err := h.parseSubscribeResponseMessage(subscribeMessage)
		return resp, responseMessage, err
	}
	if err := json.Unmarshal(message, &tickersMessage); err == nil && tickersMessage.Action == "snapshot" {
