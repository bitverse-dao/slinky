...
func TestHandlerMessage(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			msg: func() []byte {
				msg := bitverse.SubscriptionResponse{
					ReqID:   "1704449555000",
					Success: true,
					ConnId:  "0667f8fffe72e041-00000ffd-00060236-95bd0a8988d23b1a-6c176b2f",
					Data: struct {
						FailTopics    []string `json:"fail_topics"`
						SuccessTopics []string `json:"success_topics"`
					}{
						FailTopics:    []string{},
						SuccessTopics: []string{"tickers.SATS-USD", "tickers.BTC-USD"},
					},
					Type:   "COMMAND_RESP",
					RetMsg: "",
				}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			wsHandler, err := bitverse.NewWebSocketDataHandler(logger, marketConfig, bitverse.DefaultWebSocketConfig)
			require.NoError(t, err)
