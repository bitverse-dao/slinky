...
func (h *WebSocketHandler) HandleMessage(
	...
	if err := json.Unmarshal(message, &subscribeMessage); err == nil && subscribeMessage.Code != 0 {
		return resp, nil, fmt.Errorf(subscribeMessage.Message)
	}

	if err := json.Unmarshal(message, &tickersMessage); err == nil && tickersMessage.Topic == string(TickersChannel) {
